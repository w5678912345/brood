...
class Computer < ActiveRecord::Base
  ...
  Btns = { "pass"=>"审核通过", "refuse"=>"拒绝通过","clear_bind_accounts" => "解绑账号", "bind_accounts" => "分配账号","task"=>"远程任务","auto_binding_account"=>"自动绑定账号","swap_account"=>"转移账号","set_group"=>"设置分组","set_status"=>"设置状态","set_max_accounts"=>"设置最大账号数","set_allowed_new"=>"设置是否自动绑定新号"}
  ...
  def auto_bind_accounts opts
    ...
    # 查询可以绑定的账户
    accounts = Account.waiting_bind_scope.where("normal_at <= ?",Time.now).reorder("normal_at asc")
    if self.allowed_new
