...
class Computer < ActiveRecord::Base
  ...
  Btns = { "pass"=>"审核通过", "refuse"=>"拒绝通过","clear_bind_accounts" => "解绑账号", 
    ...
    "set_max_accounts"=>"设置最大账号数","set_allowed_new"=>"设置是否自动绑定新号","get_log_file"=>"提取日志文件",
  ...
  def self.include_bind_account_count
      joins(
       %{
         LEFT OUTER JOIN (
           SELECT b1.bind_computer_id, COUNT(b1.id) bind_account_count
           FROM accounts b1
           GROUP BY b1.bind_computer_id
         ) a2 ON a2.bind_computer_id = computers.id
       }
      ).select("computers.*, IFNULL(a2.bind_account_count,0) as bind_account_count")
  end
  def is_started?
  ...
  def auto_bind_accounts opts
    ...
    if opts[:allowed_server_diff].present?
      server_diff =  !opts[:allowed_server_diff]
    else
      server_diff = Setting.auto_bind_no_server_diff
    end
  ...
  def self.reset_accounts_count
      ...
      computers.each do |c|
        c.update_attributes(:accounts_count => c.bind_account_count) if c.accounts_count != c.bind_account_count
      end
  end
