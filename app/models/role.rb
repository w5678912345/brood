...
class Role < ActiveRecord::Base
  ...
  scope :today_valid,where("roles.status = 'normal' and roles.today_success = 0 and roles.vit_power > 5").
  ...
  def api_pay opts
    ...
    self.transaction do
      ...
      # end
      # 修改会话
      if (opts[:pay_type] == 'auction' or opts[:pay_type] == 'mail' or opts[:pay_type] == 'enchant') 
        account.update_attributes :today_pay_count => account.today_pay_count + 1
        if self.role_session
