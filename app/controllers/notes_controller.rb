...
class NotesController < ApplicationController
	...
	def group_by_msg
		...
		@records = Note.list_search(params).where("api_name in (?)",["exception","disconnect","discardfordays"]).select("count(id) as num,SUBSTRING_INDEX(msg,'/',1) as map").group("map").reorder("num desc")
	end
