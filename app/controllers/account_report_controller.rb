...
class AccountReportController < ActionController::Base
	def show
		...
		@records = Account.select("server,
			...
			COUNT(if(status <> 'discardfordays' and status <> 'delaycreate' and status <> 'normal' and status <> 'bslocked' and status <> 'discardforyears' and status <> 'discardbysailia',no,null)) as other_count
			").group("server")
