...
class Api::ComputersController < Api::BaseController
	...
	def reg
		...
		unless @computer
			...
			@code = 1 if @computer.update_attributes(:hostname => params[:hostname],:server=>params[:server].lstrip.rstrip,:version=>params[:version]||"default",:status=>1)
		end
	end
    ...
    @code = 1 if @computer.update_attributes(:server=>params[:server].lstrip.rstrip,:updated_at=>Time.now)
